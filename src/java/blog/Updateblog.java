...
public class Updateblog extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int c=st.executeUpdate("update blog set title='"+title+"',bimage='"+pic+"',username='"+un+"',bdate='"+dat+"',folder='"+fol+"',tag='"+tag+"',message='"+msg+"' where bid='"+bid+"'");
            
            if (c==1) {
                response.sendRedirect("editblog.jsp?ms=2");
            } else {
                response.sendRedirect("editblog.jsp?ms=3");
            }
